"""Pure dispatch feature encoding; no training, I/O, dependencies or coercion."""
import math

FEATURE_CONTRACT_VERSION = "dispatch-acceptance-features-v1"
ORDERED_FEATURES = (
...
def encode_dispatch_features(row):
    """Return ten features; expected validation failures raise allowlisted ValueError."""
    if not isinstance(row, dict) or set(row) != {"riderDistanceKm", "activeWorkload", "hourOfDay", "dayOfWeek"}:
        raise ValueError("invalid_shape")
    distance, count, hour, day = (row[name] for name in ("riderDistanceKm", "activeWorkload", "hourOfDay", "dayOfWeek"))
