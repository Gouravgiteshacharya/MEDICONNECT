import { disabledDispatchRuntime, observeDispatchShadow, type DispatchShadowDependencies, type DispatchShadowObservation } from "../ml/dispatch-runtime.js";
...
export interface DispatchOptions extends DispatchConfig, DispatchShadowDependencies { offerTimeoutMs: number; freshnessThresholdMs: number; mlModel: LogisticsModel | null; maxPredictionMinutes: number; timezoneOffsetMinutes: number; now: () => Date; }
...
export async function dispatchOrder(store: DispatchStore, orderId: string, options: DispatchOptions) {
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      let shadowObservation: DispatchShadowObservation | undefined;
      const result = await store.$transaction(async (tx) => {
        ...
        const dispatchRoundId = randomUUID();
        const deterministicRanks = new Map(deterministic.map((candidate, index) => [candidate.riderId, index + 1]));
        ...
        shadowObservation = observeDispatchShadow(options.dispatchShadowRuntime ?? disabledDispatchRuntime,
          deterministic.map((candidate, index) => ({ candidateKey: `candidate-${index + 1}`,
            riderDistanceKm: candidate.distanceKm, activeWorkload: candidate.workload, attemptedAt: new Date(now.getTime()) })),
          deterministicRanks.get(ranked[0].riderId)!, mode);
        await tx.dispatchAttempt.createMany({ data: ranked.map((candidate) => ({ orderId, riderId: candidate.riderId,
          dispatchRoundId, deterministicRank: deterministicRanks.get(candidate.riderId),
          dispatchPolicyVersion: "deterministic-dispatch-v1", selectionPolicy: mode,
          legacyModelVersion: mode === "ML_ASSISTED" ? modelVersion : null,
          workloadPenaltyKm: options.workloadPenaltyKm, shortlistSize: options.maxCandidates,
          searchRadiusKm: options.maxRadiusKm, freshnessThresholdMs: options.freshnessThresholdMs,
          suitabilityScore: candidate.predictedCompletionMinutes ?? candidate.deterministicScore, riderDistanceToPharmacyKm: candidate.distanceKm, workloadSignal: candidate.workload, routeCompatibilityScore: candidate.deterministicScore, status: "CANDIDATE", attemptedAt: now })) });
        const winner = ranked[0];
        const assignment = await tx.deliveryAssignment.create({ data: { orderId, riderId: winner.riderId, status: "OFFERED", assignmentScore: winner.predictedCompletionMinutes ?? winner.deterministicScore, assignedAt: now, offerExpiresAt: assignmentExpiresAt(now, options.offerTimeoutMs) }, select: assignmentProjection });
        await tx.dispatchAttempt.updateMany({ where: { dispatchRoundId, riderId: winner.riderId, status: "CANDIDATE" }, data: { status: "OFFERED", assignmentId: assignment.id } });
      ...
      // Transaction resolved successfully: no observations from rolled-back rounds.
      if (shadowObservation && options.onDispatchShadowResult) {
        try { await options.onDispatchShadowResult(shadowObservation); } catch { /* Observation cannot fail dispatch or trigger a retry. */ }
      }
      return result;
    } catch (error) {
