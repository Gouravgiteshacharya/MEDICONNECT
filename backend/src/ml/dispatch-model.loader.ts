...
import { validateDispatchModelArtifact, type DispatchModelArtifact } from "./dispatch-model.types.js";

export interface DispatchArtifactReader {
  readFile(path: string, encoding: "utf8"): Promise<string>;
}
export type DispatchArtifactLoadResult =
  | { readonly status: "loaded"; readonly artifact: DispatchModelArtifact }
...
export async function loadDispatchModelArtifact(
  path: string, reader: DispatchArtifactReader = { readFile },
  options: { readonly maxBytes?: number; readonly expectedSha256?: string } = {},
): Promise<DispatchArtifactLoadResult> {
  if (typeof path !== "string" || !path.trim() || path.includes("\0") || (/^[a-z][a-z0-9+.-]*:/i.test(path) && !/^[a-z]:[\\/]/i.test(path))) {
    return { status: "unavailable", reason: "invalid_path" };
  }
  const maxBytes = options.maxBytes === undefined ? 65_536 : options.maxBytes;
  if (options.expectedSha256 !== undefined && (typeof options.expectedSha256 !== "string" || !/^[a-fA-F0-9]{64}$/.test(options.expectedSha256))) return { status: "unavailable", reason: "invalid_expected_checksum" };
  ...
  const validation = validateDispatchModelArtifact(value);
