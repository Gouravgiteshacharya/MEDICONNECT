...
export function createDashboardRouter(store: DashboardStore, authenticate: Authenticator, options: DashboardOptions): Router {
  const router = Router();
  router.get("/me/dashboard", authenticate, requireAuthentication, requireRole("DELIVERY_PARTNER"), async (req, res) => res.json({ data: await getRiderDashboard(store, req.auth!.userId, options) }));
  return router;
}
