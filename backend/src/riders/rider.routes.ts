...
export function createRiderRouter(store: RiderStore & LocationStore, authenticate: Authenticator, options: RiderRouterOptions): Router {
  ...
  router.get("/me", async (req, res) => res.json({ data: await getRiderProfile(store, req.user!.id) }));
  router.patch("/me/availability", async (req, res) => {
    ...
    res.json({ data: await setRiderAvailability(store, req.user!.id, availability as ManualAvailability) });
  });
  router.patch("/me/location", async (req, res) => {
    const result = await updateRiderLocation(store, req.user!.id, parseLocationInput(req.body), options);
