...
export function createDeliveryQuoteRouter(
  ...
  options: { config: DeliveryQuoteConfig; distanceProvider: DistanceProvider; freshnessThresholdMs: number; mlModel: LogisticsModel | null; maxPredictionMinutes: number; fallbackSpeedKmh: number; timezoneOffsetMinutes: number; now: () => Date },
): Router {
  const router = Router();
